...
Buddy::Buddy(const char* name, void* start_addr, size_t total_pages)
  ...
  while (total_pages > 0) {
    ...
    void* addr =
        (char*)start_addr + (total_pages - maxLengthBlocks) * kPageSize;
    ...
    *(void**)addr = nullptr;
    ...
    if (total_pages > 0) {
      ...
      // 找到不超过剩余块数的最大2的幂
      while (true) {
        if (i <= total_pages && 2 * i > total_pages) {
          break;
        }
        i = i * 2;
        maxLength++;
...
auto Buddy::Alloc(size_t order) -> void* {
  ...
  // 情况1：直接有合适大小的空闲块
  if (freeList_[order] != nullptr) {
    // 从空闲链表头部取出一个块
    returningSpace = freeList_[order];
    // 更新链表头
    freeList_[order] = *(void**)returningSpace;
    // 清空返回块的next指针
    *(void**)returningSpace = nullptr;
  } else {
    ...
    for (auto i = order + 1; i < length_; i++) {
      if (freeList_[i] != nullptr) {
        // 找到一个更大的块，将其分割
        // 取出大块
        void* ptr1 = freeList_[i];
        // 更新大块链表
        freeList_[i] = *(void**)ptr1;
        // 计算分割后的第二个块地址
...
void Buddy::Free(void* addr, size_t order) {
  ...
  // 情况1：该大小的空闲链表为空，直接插入
  if (freeList_[order] == nullptr) {
    freeList_[order] = addr;
    *(void**)addr = nullptr;
  } else {
    // 情况2：尝试与相邻的buddy块合并
    void* prev = nullptr;
    void* curr = freeList_[order];
    ...
    while (curr != nullptr) {
      ...
      if (curr == (void*)((char*)addr + kPageSize * bNum)) {
        ...
        if (isValid(addr, order + 1)) {
          // 从链表中移除找到的buddy块
          if (prev == nullptr) {
            freeList_[order] = *(void**)freeList_[order];
          } else {
            *(void**)prev = *(void**)curr;
          }
